// // server.js - UPDATED VERSION (Network routes integrated directly)
...
//   try {
//     const client = new MongoClient(process.env.mongodb+srv//SWISH:swish2025@cluster0.b7mdrky.mongodb.net/swish_db?appName=Cluster0);
...
//   try {
//     const decoded = jwt.verify(token, process.env.swish_campus_secret_2025_change_this_in_production);
//     req.user = decoded;
//     next();
//   } catch (error) {
...
//   try {
//     const decoded = jwt.verify(token.replace('Bearer ', ''), process.env.swish_campus_secret_2025_change_this_in_production);
...
//       process.env.swish_campus_secret_2025_change_this_in_production,
//       { expiresIn: '7d' }
//     );
...
//     // Ensure network fields exist
//     userResponse.incomingRequests = userResponse.incomingRequests || [];
//     userResponse.outgoingRequests = userResponse.outgoingRequests || [];
...
//     const hasOutgoingRequest = (currentUser.outgoingRequests || []).includes(targetUserId);
...
//     const hasIncomingRequest = (currentUser.incomingRequests || []).includes(targetUserId);
//     if (hasIncomingRequest) {
//       return res.status(400).json({ message: "This user has already sent you a connection request. Please check your incoming requests." });
//     }

//     // Update both users
//     await db.collection('users').updateOne(
//       { _id: new ObjectId(currentUserId) },
//       { $push: { outgoingRequests: targetUserId } }
//     );

//     await db.collection('users').updateOne(
//       { _id: new ObjectId(targetUserId) },
//       { $push: { incomingRequests: currentUserId } }
//     );
...
//     const hasIncomingRequest = (currentUser.incomingRequests || []).includes(senderUserId);
//     if (!hasIncomingRequest) {
...
//       { _id: new ObjectId(currentUserId) },
//       { 
//         $pull: { incomingRequests: senderUserId },
...
//     await db.collection('users').updateOne(
//       { _id: new ObjectId(currentUserId) },
//       { $pull: { outgoingRequests: targetUserId } }
//     );

//     await db.collection('users').updateOne(
//       { _id: new ObjectId(targetUserId) },
//       { $pull: { incomingRequests: currentUserId } }
//     );
...
//       status,
//       message,
//       canSendRequest: !isConnected && !hasOutgoingRequest && !hasIncomingRequest
//     });
//   } catch (error) {
...
// app.get("/api/network/requests/incoming", auth, async (req, res) => {
//   try {
//     const userId = req.user.userId;
...
//     const requestIds = user?.incomingRequests || [];
...
// app.get("/api/network/requests/outgoing", auth, async (req, res) => {
//   try {
//     const userId = req.user.userId;

//     const user = await db.collection('users').findOne(
//       { _id: new ObjectId(userId) },
//       { projection: { outgoingRequests: 1 } }
//     );

//     const requestIds = user?.outgoingRequests || [];
...
//       { _id: new ObjectId(currentUserId) },
//       { projection: { connections: 1, incomingRequests: 1, outgoingRequests: 1 } }
//     );
...
//       const incomingIds = (currentUser?.incomingRequests || []).map(id => new ObjectId(id));
//       const outgoingIds = (currentUser?.outgoingRequests || []).map(id => new ObjectId(id));
      
//       excludeIds.push(...connectionIds, ...incomingIds, ...outgoingIds);
//     }
...
//       const hasOutgoingRequest = (currentUser?.outgoingRequests || []).includes(userIdStr);
//       const hasIncomingRequest = (currentUser?.incomingRequests || []).includes(userIdStr);

//       let connectionStatus = "none";
//       if (isConnected) connectionStatus = "connected";
//       else if (hasOutgoingRequest) connectionStatus = "request_sent";
//       else if (hasIncomingRequest) connectionStatus = "request_received";
...
//         incomingRequests: user?.incomingRequests?.length || 0,
//         outgoingRequests: user?.outgoingRequests?.length || 0,
